...
export default (props) => {
  ...
  const forgotPassCloseHandler = () => {
    dispatch(uiActions.closeForgotPass());
  };

  const submitForgotPassHandler = async (event) => {
    ...
    const response = await fetch(`${SERVER_URL}/api/v1/users/forgotPassword`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div>
      <Modal onClose={forgotPassCloseHandler}>
        <div className={stylesAuthForm['auth--form-container']}>
          {response && <Response>{response}</Response>}
          <h2 className={stylesAuthForm['auth--form-heading']}>Forgot Password</h2>
          <form onSubmit={submitForgotPassHandler} className={stylesAuthForm['auth--form']}>
          ...
          <CancelInput onClick={forgotPassCloseHandler} />
        </div>
      </Modal>
    </div>
  );
